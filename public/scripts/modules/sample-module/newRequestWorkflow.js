...
define(['angular',
    ...
    controllers.controller('NewrequestWorkflowCtrl', ['$state', '$http', '$log', 'commonServices', '$rootScope', '$scope', '$timeout', function($state, $http, $log, commonServices, $rootScope, $scope, $timeout) {
        var urlPathArr = [];
  ...
  var ssoObj = window.localStorage.getItem("SSO_ID");
        ...
        function isContractorSurveAssign(a_contractor, a_row) {

            if ($scope.contractorid === 'No Contractor') {
        ...
        document.getElementById("mytable").addEventListener("px-row-click", function(e) {
            ...
            if (clickedRow._selected === false) {
               ...
               // $scope.surveyTypeSelected = e.detail.row.row.surveyType.value;
                ...
                var isContinue = isContractorSurveAssign($scope.contractorid, clickedRow); //Function Call
                if (isContinue) {
                    //alert("iNSIDE cont");
                    $scope.ticketData = {
        ...
        myButton.addEventListener('tap', function(e) {
            ...
            if ($scope.multiTicket.length != 0) {
                var data = $scope.multiTicket[0];
                //console.log(data);
                console.log(JSON.stringify(data));
                // For Current Testing
				
				$http.post('https://dt-admin-microservice-dev-wf.run.aws-usw02-pr.ice.predix.io/workflow/saveTicketDetails',data)
