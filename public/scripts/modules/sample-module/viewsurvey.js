...
function(angular, controllers) 
{
	'use strict';
			...
			controllers.controller('surveyReportViewCtrl',['$http','$log','PredixAssetService','$rootScope','$scope',function($http, $log, PredixAssetService,$rootScope, $scope){
...
$http.get(getSiteList).success(function(data){
                      ...
                      console.log(data);
					   $('#btnExport').click(function(){
							...
							JSONToCSVConvertor(data, "Survey", true);
						});
						function JSONToCSVConvertor(JSONData, ReportTitle, ShowLabel) 
						{
							...
							for (var i = 0; i < arrData.length; i++) {
								...
								for (var index in arrData[i]) { console.log(JSON.stringify(arrData[i]));
									row += '"' + arrData[i][index] + '",';
								}
