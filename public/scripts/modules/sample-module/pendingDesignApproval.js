...
define(
    ...
    function(angular, controllers) {
        ...
        controllers.controller('pendingDesignApprovalCtrl', ['$state', '$timeout', '$http', '$log', 'PredixAssetService', '$rootScope', '$scope', 'commonServices', function($state, $timeout, $http, $log, PredixAssetService, $rootScope, $scope, commonServices) {
            var urlPathArr = [];
   ...
   var ssoObj = window.localStorage.getItem("SSO_ID");
        const ssoPrivilegeData = {
            ...
            var ssoObj = window.localStorage.getItem("SSO_ID");
            //var getSiteList = commonServices.getadminServiceURL()+"/sitelist?assignedTo="+ssoObj+"&stageName=PM%20Design%20Approval&status=InProgress";
            var getSiteName = "https://predix-asset.run.aws-usw02-pr.ice.predix.io/site?filter=";
            //var getSiteListSurveyor =commonServices.getadminServiceURL()+"/sitelist?assignedTo="+ssoObj+"&stageName=PM%20Design%20Approval&status=Completed";

            // LOCAL URL
            // var getSiteList = "http://localhost:8080/sitelist?assignedTo=1&stageName=PM%20Design%20Approval&status=InProgress";
            // var getSiteName = "https://predix-asset.run.aws-usw02-pr.ice.predix.io/site?filter=";
            // var getSiteListSurveyor ="http://localhost:8080/sitelist?assignedTo=1&stageName=PM%20Design%20Approval&status=Completed";

            //Hard coded url
           // var getSiteListSurveyor = commonServices.getadminServiceURL() + "/sitelist?assignedTo=" + ssoObj + "&stageName=PM%20Design%20Approval,PM%20Design%20Approval&status=InProgress,Completed";
            //var getSiteList = commonServices.getadminServiceURL() + "/sitelist?assignedTo=" + ssoObj + "&stageName=PM%20Design%20Approval&status=InProgress";
            ...
            //     $http.get(getSiteListSurveyor).success(function(data) {
                ...
                commonServices.getSiteListForPM(ssoObj, "PM Design Approval,PM Design Approval","InProgress,Completed").then(function(data) {
                    console.log("Inside service getSiteListForPM 138",data);
                    //pmList = data;
                    console.log("pmlist data 138",data)
                    //commonServices.getSiteListForPM(ssoObj, "PM Design Approval","Completed").then(function(data) {
                    //data = data.concat(pmList);
                    //console.log("inside data.concat(pmList) 276",data);
                    ...
                    angular.forEach(data, function(value, key) {
                        ...
                        pmviewData[key].ticketId = "<a href=/pmviewDesign/" + value.ticketId + "/" + siteId + "/" + value.ticketSiteDetailsId + " ui-sref=pmviewDesign({ticketId:'" + value.ticketId + "/" + value.siteId + "/" + value.ticketSiteDetailsId + "'}) 'style='text-decoration: none'>" + value.ticketId + "</a>";
